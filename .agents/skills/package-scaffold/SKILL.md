...
5. Name publish tags with the `laravel-mailmanager-*` convention so consumers can target individual resource groups.
...
- Add a publishable migration: place the migration in `database/migrations`, wire it through a console-guarded `publishesMigrations` call with a `laravel-mailmanager-migrations` tag, and test publish behavior with Testbench.
- Wire a new publish tag by adding a `publishes` map inside the existing console-guarded publishing method and naming the tag with `laravel-mailmanager-*`.
